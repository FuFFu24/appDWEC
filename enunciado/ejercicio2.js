...
function contratar(button) {
  ...
  if (!isNaN(cantidad) && cantidad > 0) {
    ...
    const foto = fotoLinkEntero.replace(
      "http://localhost:8080/enunciado/pagina2_files/",
      ""
    );
...
function mostrarCompras() {
  ...
  const ventana = window.open("", "COMPRA", "width=600,height=400");
  ventana.document.write("<h1>COMPRA</h1>");
  ventana.document.write("<p>IMAGEN --- OPERACION --- CANTIDAD</p>");
  for (const compra of compras) {
    ventana.document.write(
      `<p>${compra.foto} --- ${compra.operacion} --- ${compra.cantidad}</p>`
    );
  }
...
function mostrarAlquileres() {
  ...
  const ventana = window.open("", "ALQUILER", "width=600,height=400");
  ventana.document.write("<h1>ALQUILER</h1>");
  ventana.document.write(
    "<p>IMAGEN --- OPERACION --- CANTIDAD --- DURACION</p>"
  );
  for (const alquiler of alquileres) {
    ventana.document.write(
      `<p>${alquiler.foto} --- ${alquiler.operacion} --- ${alquiler.cantidad} --- ${alquiler.duracion}</p>`
    );
  }
  ventana.document.write('<button onclick="window.close()">Cerrar</button>');
}
...
function mostrarTodasLasOperaciones() {
  ...
  for (const operacion of operaciones) {
    if (compras.find((c) => c.foto === operacion.foto)) {
      ventana.document.write(
        `${operacion.foto} --- ${operacion.operacion} --- ${operacion.cantidad}</p>`
      );
    } else {
      ventana.document.write(
        `${operacion.foto} --- ${operacion.operacion} --- ${operacion.cantidad} --- ${operacion.duracion}</p>`
      );
    }
  }
  ventana.document.write('<button onclick="window.close()">Cerrar</button>');
...
window.onload = function () {
  ...
  const verCompras = document
    .querySelector("#derecha")
    .querySelectorAll("a")[0];
  const verAlquileres = document
    .querySelector("#derecha")
    .querySelectorAll("a")[1];
  const verOperaciones = document
    .querySelector("#derecha")
    .querySelectorAll("a")[2];

  verCompras.addEventListener("click", function (event) {
    event.preventDefault();
    mostrarCompras();
  });

  verAlquileres.addEventListener("click", function (event) {
    event.preventDefault();
    mostrarAlquileres();
  });

  verOperaciones.addEventListener("click", function (event) {
    event.preventDefault();
